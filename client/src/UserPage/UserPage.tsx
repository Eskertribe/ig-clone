...
const UserPage: React.FC = () => {
  ...
  const [showModal, toggleModal] = useState({ show: false, post: undefined, image: undefined });
      ...
      <div className="bg-gray-200 h-screen flex flex-col">
        ...
        <div className="h-[80vh] grid grid-rows-3 grid-cols-3 gap-1 p-2">
          {userData?.posts?.map((post) => (
            // @ts-ignore
            <Post key={post.id} post={post} toggleModal={({ post, image }) => toggleModal({ show: !showModal.show, post, image })} />
          ))}
        </div>
      </div >
      {showModal.show && showModal.post && <PostModal
        isOpen={showModal.show}
        // @ts-ignore
        // TODO: Fix this type error
        imageUrl={showModal.image ? showModal.image : undefined}
        onClose={() => toggleModal({ show: !showModal.show, post: undefined, image: undefined })}
      />}
    </>
  )
